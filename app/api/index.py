...
# ────────────────  Credenciales DEMO (brand1 … brand10)  ───────────────────
...
RAW_EVENTS = [
    ...
    # CLARINS (algunas sin fecha → no se cargan)
...
def parse_spanish_date(s: str) -> str:
    """Convierte '06-jul-25' → '2025-07-06' (ISO-8601)."""
    d, m, yy = s.split('-')
...
def load_events_manual():
    ...
    for prov, marca, pais, fecha_str in RAW_EVENTS:
        if not fecha_str:      # tupla “sin fecha” → se ignora
            continue
        evs.append({
            "proveedor": prov,
            "pais":      pais,
            "marca":     marca,
            "user":      "brand1",            # usuario demo
